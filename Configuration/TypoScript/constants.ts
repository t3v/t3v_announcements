...
plugin {
  tx_t3vannouncements {
    persistence {
      ...
      # cat=plugin/tx_t3vannouncements/persistence; type=int+; label=The PID of the default storage
      storagePid = 0

      # cat=plugin/tx_t3vannouncements/persistence; type=boolean; label=Updates the reference index to ensure data integrity
      updateReferenceIndex = 1
    }

    settings {
      extbase {
        # cat=plugin/tx_t3vannouncements/settings/extbase; type=string; label=The controller extension name
        controllerExtensionName = T3vAnnouncements
      }
    }
